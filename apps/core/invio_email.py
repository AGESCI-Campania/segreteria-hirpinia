...
`apps/note_spese/promemoria.py`, `apps/note_spese/report_gestori.py`)
devono passare invece di chiamare `render_to_string`/`send_mail`
direttamente. Non
...
_FALLBACK: dict[str, tuple[str, str]] = {
    ...
    CodiceTemplateEmail.NOTA_SPESE_REPORT_GESTORI: (
        "Catello — report note spese in sospeso",
        "note_spese/email/nota_report_gestori.txt",
    ),
}

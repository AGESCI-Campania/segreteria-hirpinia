...
def breadcrumb(request: HttpRequest) -> dict:
    ...
    presente Home, con Sezione › Voce (o Sezione › Voce › Sottovoce per un
    modulo con più funzioni, es. Nota Spese) quando `request.path`
    corrisponde esattamente a una voce o sottovoce di menu. Le pagine
    "figlie" non presenti nel menu (dettaglio, creazione, conferma, ecc.)
    mostrano solo Home, salvo che la view implementi `BreadcrumbExtraMixin`
    (`apps/core/mixins.py`): in quel caso si accodano i suoi item —
    un'estensione esplicita, non un secondo meccanismo che duplica la logica
    sezione/voce sopra."""
    ...
    for sezione in sezioni_menu(request.user):
        for voce in sezione.voci:
            ...
            for sotto in voce.sottovoci:
                if sotto.url == request.path:
                    items.append({"label": sezione.etichetta})
                    items.append({"label": voce.etichetta, "url": voce.url})
                    items.append({"label": sotto.etichetta})
                    return {"breadcrumb_items": items}
